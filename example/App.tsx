...
const App = () => {
  ...
  const onChange = (color: string) => {
    ...
    document.body.style.setProperty('--text-color', (newColor.isDark && newColor.a > 0.33) ? 'rgba(255,255,255,0.75)' : 'rgba(0,0,0,0.7)');
    document.body.style.setProperty('--heading-color', (newColor.isDark && newColor.a > 0.33) ? 'rgba(255,255,255,0.95)' : 'rgba(0,0,0,0.85)');
    document.body.style.setProperty('--bg-color', (newColor.isDark && newColor.a > 0.33) ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.10)');
    // console.log({ color, newColor: newColor.toString(), isDark: newColor.isDark && newColor.a > 0.33, name: newColor.name })
  };
  return (
    <>
    ...
    <header class='col col-lg-6 mx-auto mb-5' style={{ 'padding-top': '15vh' }}>
    ...
    <main class="container">
        <div class="row">
          <div class="col col-lg-6 mx-auto">
            <label class="visually-hidden" for="my-id-red">Default Color Picker</label>
            <DefaultColorPicker
              id="my-id-red"
              format={format()}
              value={'red'}
        ...
        <div class="row">
          <div class="col col-lg-6 mx-auto">
            <div class='btn-toolbar mb-3'>
              <button class={`btn${format() === 'rgb' ? ' active' : ''}`} onClick={() => setFormat('rgb')}>RGB</button>
              <button class={`btn${format() === 'hsl' ? ' active' : ''}`} onClick={() => setFormat('hsl')}>HSL</button>
              <button class={`btn${format() === 'hwb' ? ' active' : ''}`} onClick={() => setFormat('hwb')}>HWB</button>
              <button class={`btn${format() === 'hex' ? ' active' : ''}`} onClick={() => setFormat('hex')}>HEX</button>
            </div>
          </div>
        </div>
      </main>
        ...
        <div class='col col-lg-6 mx-auto'>
          <div class='d-flex justify-content-between'>
            ...
            <p class="links">
              ...
              {' / '}
              <a target="_blank" href="https://www.npmjs.com/package/@thednp/solid-color-picker" title="Solid Color Picker on NPM">NPM</a>
            </p>
          </div>
        </div>
      </footer>
